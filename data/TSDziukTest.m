...
% project point on to the surface, this from is part of iFEM by Long Chen
    ...
    function rhs = f(p,t) % load data (right hand side function)
        x = p(:,1); y = p(:,2); z = p(:,3);
        phi = [x-z.^2, y, z - 2*z.*(x-z.^2)];
        absphi = sqrt(sum(phi.^2,2));
        n = phi./absphi;
        dnxdx = (absphi.^2 - (x-z.^2).^2)./absphi.^3;
        dnydy = (absphi.^2 - y.^2)./absphi.^3;
        dnzdz = (absphi.^2 - (z-2*z.*x.^2).*(1-x+6*z.^2))./absphi.^3;
        Du = zeros(size(p));
        Du(:,1)=1; Du(:,2)=0;Du(:,3)=0;
        rhs = sum(Du.*n,2).*(dnxdx + dnydy + dnzdz);
        rhs = rhs.*exp(t) ;
    end
% exact solution
